<template>
  <MarkdownRenderer v-if="normalizedFormat === 'markdown'" :markdown="content" />
  <div v-else-if="normalizedFormat === 'html'" class="docs-markdown" v-html="sanitizedHtml"></div>
  <pre v-else class="whitespace-pre-wrap text-sm leading-6 text-gray-700 dark:text-dark-200">{{ content }}</pre>
</template>
...
const normalizedFormat = computed(() => String(props.format || 'markdown').toLowerCase())
