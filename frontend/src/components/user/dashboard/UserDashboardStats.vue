<template>
  <div class="space-y-4">
    <!-- Row 1: Primary Stats (2 large cards) - Key metrics with enhanced styling -->
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <!-- Balance (Primary) -->
      <div v-if="!isSimple" class="stat-card stat-card-primary p-5 items-center">
        ...
        <div>
          <p class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ t('dashboard.balance') }}</p>
          <p class="text-3xl font-bold text-emerald-600 dark:text-emerald-400">${{ formatBalance(balance) }}</p>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t('common.available') }}</p>
        </div>
      </div>

      <!-- Today Cost (Primary) -->
      <div class="stat-card stat-card-primary p-5 items-center">
        <div class="stat-card-glow"></div>
        <div class="stat-icon bg-gold-100 text-gold-700 dark:bg-gold-900/25 dark:text-gold-300">
          <Icon name="dollar" size="md" :stroke-width="2" />
        </div>
        <div>
          <p class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ t('dashboard.todayCost') }}</p>
          <p class="text-3xl font-bold text-gray-900 dark:text-white">
            <span class="text-gold-700 dark:text-gold-300" :title="t('dashboard.actual')">${{ formatCost(stats?.today_actual_cost || 0) }}</span>
            <span class="text-lg font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-sm mt-1">
            <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
            <span class="text-gold-700 dark:text-gold-300 font-medium" :title="t('dashboard.actual')">${{ formatCost(stats?.total_actual_cost || 0) }}</span>
    ...
    <!-- Row 2: Secondary Stats (4 small cards) -->
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <!-- API Keys -->
      <div class="stat-card p-4 items-center card-hover">
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-2">
      <!-- Performance (RPM/TPM) -->
      <div class="stat-card p-4 items-center card-hover">
        <div class="stat-icon bg-violet-100 text-violet-600 dark:bg-violet-900/30 dark:text-violet-400">
      ...
      <!-- Avg Response Time -->
      <div class="stat-card p-4 items-center card-hover">
        <div class="stat-icon bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400">
