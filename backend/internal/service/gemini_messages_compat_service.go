	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		if err != nil {
			...
			if strings.TrimSpace(proxyURL) != "" {
				log.Printf("Gemini account %d: upstream request error via proxy, triggering failover: %s", account.ID, safeErr)
...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		if resp.StatusCode == http.StatusBadRequest {
			...
			upstreamMsg = sanitizeUpstreamErrorMessage(upstreamMsg)
			if isGoogleProjectConfigError(strings.ToLower(upstreamMsg)) {
				upstreamDetail := ""
				if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
					maxBytes := s.cfg.Gateway.LogUpstreamErrorBodyMaxBytes
					if maxBytes <= 0 {
...
func sleepGeminiBackoff(attempt int) {
	delay := boundedExponentialBackoff(geminiRetryBaseDelay, geminiRetryMaxDelay, attempt)
