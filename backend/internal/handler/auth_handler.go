...
type AuthResponse struct {
	...
	ExpiresIn    int       `json:"expires_in,omitempty"` // Access token expiry (seconds)
	TokenType    string    `json:"token_type"`
	User         *dto.User `json:"user"`
}

// respondWithTokenPair generates a token pair and responds in a backwards compatible format.
// If refresh token support isn't configured, it falls back to access-token-only response.
...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	_, user, err := h.authService.RegisterWithVerification(c.Request.Context(), req.Email, req.Password, req.VerifyCode, req.PromoCode)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	_ = token // token is regenerated by respondWithTokenPair
...
func (h *AuthHandler) Logout(c *gin.Context) {
	...
	_ = c.ShouldBindJSON(&req) // allow empty body (backward compatible)
