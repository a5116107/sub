...
func TestAPIContracts(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			path:   "/api/v1/keys",
			body:   `{"name":"Key One","group_id":1,"custom_key":"sk_custom_1234567890"}`,
			headers: map[string]string{
			...
			wantJSON: `{
				...
				"data": {
					...
					"daily": {
						...
						"resets_at": "2025-01-02T00:00:00+08:00",
						"resets_in_seconds": 0
					}
				}
			}`,
		...
		},
		{
			...
			setup: func(t *testing.T, deps *contractDeps) {
				...
				deps.settingRepo.SetAll(map[string]string{
					...
					service.SettingKeyGatewayFailoverSensitive400Keywords: `[]`,
					service.SettingKeyGatewayFailoverTemporary400Keywords: `[]`,
					service.SettingKeyGatewayFailoverRequestErrorKeywords: `[]`,
					service.SettingKeyGatewayCodexModelAliases:            `{}`,
				})
			},
			method:     http.MethodGet,
...
func newContractDeps(t *testing.T) *contractDeps {
	...
	adminService := service.NewAdminService(userRepo, groupRepo, &accountRepo, proxyRepo, apiKeyRepo, redeemRepo, usageRepo, nil, nil, nil, nil, nil, nil, nil, nil, nil)
...
func (r *stubUserRepo) GetByInviteCode(ctx context.Context, inviteCode string) (*service.User, error) {
	for _, user := range r.users {
		if user.InviteCode != nil && *user.InviteCode == inviteCode {
			clone := *user
			return &clone, nil
		}
	}
...
func (r *stubUserRepo) SetInviteCodeIfEmpty(ctx context.Context, userID int64, inviteCode string) (bool, error) {
...
func (r *stubUserRepo) SetInvitedByIfEmpty(ctx context.Context, userID int64, inviterUserID int64, invitedAt time.Time) (bool, error) {
	user, ok := r.users[userID]
	if !ok {
		return false, service.ErrUserNotFound
	}
